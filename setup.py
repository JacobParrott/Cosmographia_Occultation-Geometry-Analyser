...
for i in tqdm(range(winsiz-1)):
    ...
    lon[i], lat[i], dist[i], nearestpoint, alt, speed, date_time[i], ingress_date_time[i], veldopp[i], MexNadirTGOAngle[i] = main.Location(
    ...
    hour, minute, sec, _, _ = spice.et2lst(
        occs.Time[i], 499, (lon[i] * (math.pi/180)), 'PLANETOCENTRIC')  # 499 = spice code for Mars
    localtimevect = [hour, minute, sec]
    localtime[i] = "%d:%d:%d" % (
        localtimevect[0], localtimevect[1], localtimevect[2])
...
# Will not save to exel file if it is open in another window
occs.to_excel("OccultationSelection.xlsx")
